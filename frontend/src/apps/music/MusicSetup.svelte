<script lang="ts">
  ...
  async function detectDefaults() {
    loading = true;
    try {
      defaultMusicPath = await musicSettings.detectDefaultMusicPath();
      musicPath = defaultMusicPath;
    } catch (e) {
      console.error("检测默认路径失败:", e);
    } finally {
      loading = false;
    }
  }
  ...
  function completeSetup() {
    if (!musicPath) {
      error = $t("music.selectMusicFolder");
      return;
    }
    musicSettings.completeSetup(musicPath);
    onComplete();
  }
  ...
  async function skipSetup() {
    loading = true;
    try {
      const path = defaultMusicPath || await musicSettings.detectDefaultMusicPath();
      musicSettings.completeSetup(path);
      onSkip();
    } catch (e) {
      error = $t("music.setupFailedManual");
      step = "select";
    } finally {
      loading = false;
    }
  ...
  {#if step === "welcome"}
    <div class="setup-card">
      <div class="setup-icon">
        <Icon icon="mdi:music-circle" width={72} />
      </div>
      <h1>{$t("music.welcome")}</h1>
      <p class="setup-desc">
        {$t("music.welcomeDesc")}
      </p>
      ...
      {#if loading}
        <div class="loading-hint">
          <Spinner size="sm" />
          <span>{$t("music.detecting")}</span>
        </div>
      {:else if defaultMusicPath}
        <div class="detected-path">
          <Icon icon="mdi:folder-music" width={20} />
          <span>{$t("music.detectedFolder")} <strong>{defaultMusicPath}</strong></span>
        </div>
      {/if}
  ...
  {:else if step === "select"}
    <div class="setup-card wide">
      <div class="setup-header">
        ...
        <h2>{$t("music.selectMusicFolderTitle")}</h2>
      </div>

      <p class="setup-hint">{$t("music.selectFolderHint")}</p>
      ...
      <div class="path-input-group">
        <div class="path-display">
          <Icon icon="mdi:folder-music" width={20} />
          <span class="path-text">{musicPath || $t("music.notSelected")}</span>
        </div>
      ...
      {#if defaultMusicPath && musicPath !== defaultMusicPath}
        <button class="default-hint" onclick={useDefaultPath}>
          <Icon icon="mdi:lightbulb-outline" width={16} />
          {$t("music.useDetectedPath")} {defaultMusicPath}
        </button>
      {/if}

      <div class="setup-actions">
        <Button variant="primary" size="lg" onclick={completeSetup} disabled={!musicPath}>
          <Icon icon="mdi:check" width={20} />
          {$t("music.completeSetup")}
        </Button>
      </div>
    </div>
  {/if}
